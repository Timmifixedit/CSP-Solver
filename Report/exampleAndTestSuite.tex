...
\lstinputlisting[linerange={15-29}]{../src/main.cpp}
Um die \inlcode{SudokuNode}s zu verwalten, wurde außerdem die Hilfsklasse \inlcode{Sudoku} implementiert. Deren Konstruktor ließt die gegebenen Werte aus einem \inlcode{std::istream}
...
Der gezeigte Test überprüft, ob die Methode \inlcode{util::removeInconsistent}, die im \ac*{ac}-3 Algorithmus zum Einsatz kommt, den korrekten Wert zurück liefert. Dies lässt
sich mit den von Google Test zur Verfügung gestellten Makros wie \inlcode{EXPECT_TRUE} überprüfen. Es gibt darüber hinaus weitere Makros, die auch komplexeres Verhalten überprüfen
können.
